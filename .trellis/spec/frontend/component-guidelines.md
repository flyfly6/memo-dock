...
- Use named imports from `@tabler/icons-react` for common UI icons. Give compact Mantine controls an explicit icon `size` and `stroke`, inherit `currentColor`, and use Unicode characters only when the character is content rather than an icon.
...
- Use BlockNote's default slash menu. Disable the default two-button side menu and compose its exported `SideMenu` and `DragHandleButton` through `SideMenuController`; do not implement custom drag behavior. Checklist completion ordering follows the executable contract in `state-management.md`.
- Register `Mod-Alt-Enter` through BlockNote `createExtension` to insert an unchecked `checkListItem` before the current block and focus it. Use public `insertBlocks` and `setTextCursorPosition` APIs; do not override the built-in Enter or block-move shortcuts.
